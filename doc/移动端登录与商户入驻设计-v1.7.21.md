...
| v1.7.21 | 2026-09-25 | 关于页重设计（用户验收反馈）：品牌区纯 View 绘制真 app-icon 造型（蓝底圆+白环+6 钢珠，规避 RN 不支持 svg 文件与 gradient 两坑）+应用名 OpenFindBearings+版本；功能卡仅检查更新与备案信息（协议/隐私入口在设置页隐私组不重复放）；复用 settings.scss 卡片样式修复首版裸布局；底部简介+版权行；应用名与公司名暂用 OpenFindBearings 占位 |
| v1.7.21 | 2026-09-25 | 寻货上线：发现 tab 转正为寻货 feed（型号搜索+全部/等待应答过滤+右上发布入口）；寻货详情页（需求卡+发布人比价列表+商户内嵌应答表单+选定/取消+解锁联系方式复制卡，可见性分级防报价泄露）；发布寻货页（表单+NEED_POINTS 积分确认弹窗）；我的寻货页（八格「我的寻货」转正）；商家寻货应答页（商家 Tab 功能区新按钮，员工可见——应答是商家行为）；services/sourcing.ts 额度协议解析 |
| v1.7.21b | 2026-09-25 | 设置页分组定稿：权限管理并入隐私组（授权属隐私语义）；新增「帮助」组=自动更新开关（留在设置页，管是否启用）+服务热线（从其他组挪入）+关于入口；新建关于二级页 pages/my/about=品牌区+版本+手动检查更新+ICP 备案（用户定案：外面管开关、关于里只管手动检查）；备案行此前已实施，显示前提是 Admin 系统配置 Site.BeiAn 填值（空则隐藏） |
| v1.7.21 | 2026-09-25 | 任务中心与设置页重排：① 新增任务中心页 pages/my/tasks——每日签到 7 格日期条（今天居中、已签实心勾、漏签灰、未来空心）+ 赚分任务列表（API /points/tasks 驱动：daily 任务完成态只看今日流水、次日自动刷新回未完成，once 任务（注册/入驻）看历史永久完成；行动按钮去签到/去纠错/去入驻）；② 我的页积分卡回归原样（撤签到胶囊与连签副文案，一行摆不下显示不全；赚分动作收进任务中心），余额保留真数据；③ 功能区扩两行八格：收藏轴承/关注商家/浏览历史/我的纠错 + 我的寻货(占位)/任务中心/设置/全部功能；④ 设置页重排：音效/震动挪消息组（新消息提醒语义归位），新增权限组与权限管理二级页（相机/相册用途说明+去系统设置，openAppAuthorizeSetting 三端降级兼容），通用组改关于组（启动时自动检查更新简称自动更新、版本更新、ICP 备案行取站点配置点击复制）；⑤ 首页搜索框内扫描/麦克风/相机图标与占位色由硬编码灰改主题色（深色模式原色不可见），快捷三钮渐变底保留品牌色不变（主流做法）；⑥ 修复新建三页（积分明细/任务中心/权限管理）NavBar 缺返回箭头——showBack 默认 false 需显式开启；⑦ 任务中心行动按钮扩展：资料完善=去维护、首件上架=去上架（跳商家 Tab），配合 API v1.34.0 商户侧两条新一次性规则 |
|------|------|------|
