...
- 新增 `deploy/apk-server/apk-server.yml` 内 CronJob `openfindbearings-apk-sync`（每 30 分钟）：从 GitHub Release API 取最新规范版本（按发布时间序）→ 用资产自带 sha256 校验/`wget -c` 断点续传拉取分包到 `/dl` → 两包齐备才 psql 写 `Mobile.AppVersion`（弹窗文案为固定模板"发现新版本 X，建议更新"，Admin 可覆盖）；拉不完不宣告、下轮自愈。
- 客户端检查逻辑不变：无论落后多少个版本，只与服务端单一"最新版指针"比较、只下载最新版拼名文件，无逐版本升级概念。

### v1.3.0 (2026-09-14)

- 新增版本更新对接：`API.VERSION_CHECK = /mobile/version/check`（BFF 匿名代理到 API `/api/mobile/version/check`），`services/update.ts` 统一三端策略。
- 新增 `utils/version.ts` / `utils/version.rn.ts`：版本号单一来源——H5/小程序读编译期常量 `__APP_VERSION__`（`config/index.ts` 从 `package.json` 注入），RN 读安装包 `versionName`（`android/app/build.gradle` 亦从 `package.json` 解析）；发布只改 `package.json` 一处。
- 版本更新三端：RN/Android 走原生 `ApkUpdate` 模块（`DownloadManager` 下载 + 进度事件 `ApkUpdateProgress` + FileProvider 拉起安装）；H5 提示刷新；小程序 `getUpdateManager` 重启生效。
- 版本号比较由后端按 SemVer 2.0 完成（含 `rc.N` prerelease），前端不做比较逻辑，仅按 `hasUpdate/isForceUpdate` 展示。
- 启动静默检查每日一次（`last_version_check_ts` 节流），设置页"版本更新"行为手动检查（可提示"已是最新版本"）。
- 修复：`getSiteConfig()` 原 404（BFF 缺 `/mobile/config` 代理），本次补齐匿名代理。
...
export const API = {
  ...
  VERSION_CHECK: '/mobile/version/check',
} as const
```
...
## 版本更新（`services/update.ts`）

- `checkUpdateOnLaunch()`：启动静默检查（`appShared.bootOnce` 延迟触发），每日节流一次（本地 `last_version_check_ts`），仅发现更新才弹窗。
- `checkUpdateManually()`：设置页"版本更新"行手动检查，无更新也反馈。
- 请求：`GET /mobile/version/check?currentVersion=<本地版本>&platform=<h5|weapp|android|ios>`（`auth:false`）。

```typescript
// 响应（后端已完成 SemVer 比较，前端不做版本大小判断）
interface VersionCheckResult {
  hasUpdate: boolean
  latestVersion: string
  isForceUpdate: boolean
  updateMessage?: string | null
  downloadUrl?: string | null   // APK 下载目录（以 / 结尾）
}
```
...
| 端 | 行为 |
|----|------|
| RN/Android | `downloadUrl + app-v<版本>-<ABI>.apk` 拼完整地址 → 原生 `ApkUpdate.downloadAndInstall` → `DownloadManager` 下载（通知栏 + `ApkUpdateProgress` 事件刷新 `showLoading`）→ 完成经 FileProvider 拉起系统安装；缺"安装未知应用"权限时跳系统设置页并提示 |
| RN/iOS | 无二进制分发，`hasUpdate` 时回退 `Linking.openURL` 打开下载页 |
| H5 | 服务端部署即最新，仅提示"刷新页面"（`window.location.reload`） |
| 小程序 | `Taro.getUpdateManager()`：`onUpdateReady` 弹窗重启 `applyUpdate()` |

> 版本号单一来源：tag。发布 GitHub Release（tag `vX.Y.Z` 或 `vX.Y.Z-rc.N`）后 CI 自动：以 tag 反写工作区 package.json（不 commit）→ H5/`__APP_VERSION__`/APK `versionName` 与 tag 单源一致 → 构建 APK 传 Release。此后与集群无关——K3s 侧 CronJob `openfindbearings-apk-sync` 每 30 分钟从 Release 拉最新规范版本的分包到 `/dl`（sha256 校验、断点续传），两包齐才自动改写 `Mobile.AppVersion`、写固定模板 `Mobile.UpdateMessage`；拉不完则不宣告、下轮续。非规范 tag 的 Release 永远不会被同步宣告（纯存档）。仓库里 package.json 允许滞后一个版本；手动改配置仅作为 Admin 兜底手段。
