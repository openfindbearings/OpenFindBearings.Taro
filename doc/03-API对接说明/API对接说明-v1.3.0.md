...
> 版本号单一来源：tag。发布 GitHub Release（tag `vX.Y.Z` 或 `vX.Y.Z-rc.N`）后 CI 全自动：以 tag 反写工作区 package.json（不 commit）→ H5/`__APP_VERSION__`/APK `versionName` 与 tag 单源一致 → 构建 APK 传 Release 存档 + `kubectl cp` 推 `/dl` → 一次性 psql Pod 把 tag 写入 `Mobile.AppVersion`、Release 正文首行写入 `Mobile.UpdateMessage`。非规范 tag 只传 Release 页面、不推 `/dl` 不改配置（纯存档）。仓库里 package.json 允许滞后一个版本；手动改配置仅作为 Admin 兜底手段。
