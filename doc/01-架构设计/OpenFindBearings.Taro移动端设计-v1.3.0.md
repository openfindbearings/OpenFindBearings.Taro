# OpenFindBearings.Taro 移动端设计 v1.3.0

## 概述

OpenFindBearings.Taro 是统一的移动端前端，基于 Taro 3.6.40 + React 18 + TypeScript 构建，一套代码编译到 H5（浏览器）、微信小程序、Android/iOS 原生 APP（React Native），替代原 MAUI + 微信小程序分开做的方案。
...
- **TabBar 三格 → 五格**：首页 | 发现 | 商家 | 商城 | 我的（发现/商城为占位页，导航先行；页面布局与元素详见《界面设计规范 v1.4.0》§4.8-4.10）。
- 商户域页面成体系：入驻申请三通道（自助/认领/提名，材料矩阵上传、全量商户搜索防重复新建）、商户页（状态卡+左滑撤回/删除+邀请横幅）、商品管理（搜索选型号添加/行内编辑/上下架/Excel 导入三端）、成员管理（邀请确认制+成员详情面板+按角色命名"员工列表"）、信息维护（主体字段锁定+证照材料区）。
- 消息中心上线（站内信收件箱）+ 未读角标体系（TabBar"我的"红点与右上铃铛同源）。
- 认证闭环：申请认证端点 + Admin 审核 + 金色"已认证"徽标；信用代码入驻必填。
- 更新链路：真实版本检查 + 两段式更新弹窗 + 设置页"启动时自动检查更新"开关 + RN 应用内下载安装。
- 构建链：pnpm 统一 12.4.2（CI/Dockerfile/本地严格一致），patch 配置单一来源 pnpm-workspace.yaml。

### v1.2.0 (2026-09-03)

- 文档 Dockerfile 示例对齐实际实现（corepack prepare pnpm@9.15.0）。
- 文档 nginx.conf 示例更新（SPA 回退 + 静态资源缓存 + gzip）。
- TabBar 描述更新：浮动自定义 TabBar，已入驻商家大圆突出样式。
...
| 文档 | 说明 |
|------|------|
| [界面设计规范](./OpenFindBearings.Taro界面设计规范-v1.3.0.md) | 色彩/运行时主题(深浅×预设)/字号/组件/页面布局 |
| [API 对接说明](./01-API对接说明/API对接说明-v1.1.0.md) | 前端调用 BFF 的完整接口清单、请求/响应结构 |
| [UI 组件规范](./02-UI组件规范/UI组件规范-v1.1.0.md) | 图标抽象层、公共组件、样式变量 |
| [跨端适配说明](./03-跨端适配说明/跨端适配说明-v1.2.0.md) | 三阶段路线、RN 度量/存储/安全区/图标适配 |
| [架构重构方案](./02-架构与重构方案/Taro架构重构方案-v1.7.0.md) | RN 优先重构设计与落地记录 |
| [设置入库与合规待办](./02-架构与重构方案/设置入库与合规待办-v1.0.0.md) | 账号/合规类设置进库设计（随登录实现，待完成） |
...
## 五 Tab 架构（v1.3.0，原三 Tab 扩展）

```
┌──────────────────────────────────────────────────────────────────┐
│ 首页    发现     商家      商城     我的                        │
│ 核心查询 占位   入驻/经营  占位    个人行为                    │
│ ·搜索   求货板  ·入驻三通道 积分兑换 ·收藏/关注/历史            │
│ ·热门   (预留)  ·商品管理  (预留)  ·消息中心(角标)             │
│ ·推荐商家      ·成员邀请确认      ·会员卡/积分(预留)          │
│ ·详情×2       ·信息维护/认证                                  │
└──────────────────────────────────────────────────────────────────┘
```

发现/商城为占位页（导航先行，功能规划中；商城积分合规三纪律：不可充值/不可提现/不可转让）。
...
本 Tab 根据用户入驻状态切换视图（v1.3.0 细化，支持一人多商户 + 商户切换器）。

| 状态 | 界面 | 功能 |
|---|---|---|
| 未入驻 | 入驻引导卡 | 申请入驻三通道：自助新建/认领爬虫商户/接受提名；材料矩阵上传（营业执照全类型必备、授权经销商加品牌授权书、厂家选传厂房照）；全量商户搜索防重复新建（已入驻/审核中徽标阻断） |
| 审核中 | 状态卡 | 提交时间/预计时效；左滑撤回（SwipeCell） |
| 被拒 | 状态卡（红色） | 拒绝原因展示；点击进编辑重提；左滑删除 |
| 已入驻 | 商户卡列表 | 名称旁"已认证"金色徽标；四按钮：商品管理/成员管理（员工看为"员工列表"）/信息维护（管理员）/申请认证（未认证管理员）；待确认员工邀请横幅（接受/拒绝）；多商户切换 |

入驻后 Tab 图标从通用图标切换为商家自己的 logo（浮动自定义 TabBar，中间大圆突出显示，aspectFit 白底完整展示）。
...
| 消息中心 | 右上铃铛（未读红点，与 TabBar"我的"角标同源），站内信收件箱/已读/跳转 | 需登录 |
| 设置 | 关于、版本、检查更新（含启动自动检查开关）、清除缓存 | 无 |
...
# v1.3.0 更新：pnpm 与本地/CI 严格一致（12.4.2）；overrides/patchedDependencies 配置源为
# pnpm-workspace.yaml（v11+ 不再读 package.json 的 pnpm 节），COPY 必须带上它和 patches/，
# 否则容器内 frozen 校验 CONFIG_MISMATCH
