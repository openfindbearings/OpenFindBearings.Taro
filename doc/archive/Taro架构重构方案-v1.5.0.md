...
| RN | `CustomTabBar` 浮动组件 + Emoji 图标 | lucide-react-taro 不兼容 RN 0.70 |
| 微信小程序 | 原生 tabBar（app.config.ts 缺配置） | 缺 PNG 图标，缺配置 |

**问题**：
- RN 端 `lucide-react-taro` v1.5.2 依赖 RN 0.71+ 的 `codegenNativeCommands` API，但 Taro 3.6.40 配套 RN 0.70（不兼容）
- `lucide-react` v0.378.0 依赖 `react-native-svg` 原生模块（未装）；`import * as Lucide` 会让 Metro 在 bundling 阶段就尝试解析不存在的导出符号
- 微信小程序原生 tabBar 缺 PNG 图标资源 + 缺 app.config.ts 配置
- 三个平台图标实现方案不统一（详见 [8.10 节](#810-图标系统架构lucide-兼容性问题与-icon-抽象层)）
...
| 平台 | 选中态 | 未选中态 |
|------|--------|----------|
| H5 | Lucide 图标 + 主色 | Lucide 图标 + 灰色 |
| RN/小程序 | Emoji 主图标 | Emoji 灰色图标 |

> 详细图标分层方案见 [8.10 节](#810-图标系统架构lucide-兼容性问题与-icon-抽象层)。
...
**图标**：
- ✅ 统一用 `Icon` 组件 + props 传颜色和尺寸：`<Icon name="camera" color="#333" size={32} />`
- ❌ 禁止直接 `import { Camera } from 'lucide-react'`（RN 端会崩）
- ❌ 禁止用 className 传颜色（`className='icon-voice'`）— CSS 变量在 RN/小程序不可靠
- 颜色值用主题变量（如 `color={theme.primary}`），不硬编码
- H5/RN/小程序平台差异由 Icon 抽象层屏蔽（详见 [8.10 节](#810-图标系统架构lucide-兼容性问题与-icon-抽象层)）
...
**图标**：
- ✅ 统一用 `Icon` 抽象层组件，调用方无感知平台差异
- H5 端走 `lucide-react` 具名 import（SVG），RN/小程序端走 emoji 文本
- ❌ 禁止直接 `import { Camera } from 'lucide-react'` 或 `lucide-react-taro`（RN 端会崩）
- 详细分层方案见 [8.10 节](#810-图标系统架构lucide-兼容性问题与-icon-抽象层)
...
**结论：使用 `lucide-react-taro` + `LucideTaroProvider` 作为基础方案**，但 Taro 3.6.40 + RN 0.70 环境下 lucide 系列库存在兼容性问题，需要通过 `Icon` 抽象层屏蔽平台差异（详见 [8.10 节](#810-图标系统架构lucide-兼容性问题与-icon-抽象层)）。

**lucide-react-taro 优势**：
- Tree Shaking：每个图标独立模块，按需打包
- 与 lucide-react API 一致：`size` / `color` / `strokeWidth` 等 props
- 提供 `taro-lucide-tabbar` CLI 工具生成小程序 TabBar PNG
- 提供 `LucideTaroProvider` 全局默认色/尺寸

**pnpm install 仅装 `lucide-react-taro` 17 个版本可用**（全部 1.x 系列）：

| 版本范围 | 状态 |
|---------|------|
| 1.0.0 → 1.5.2 | 共 17 个版本，全部面向 **Taro 4.x + RN 0.73+** |
| 无 Taro 3.6.x 专用版 | 作者声称支持 Taro 3.x 但未在 RN 0.70 测过 |

### 8.10 图标系统架构（lucide 兼容性问题与 Icon 抽象层）

#### 8.10.1 背景与问题

v1.4.0 及之前版本假设 H5/RN/小程序三端可统一用 `lucide-react-taro`。实操中发现 Taro 3.6.40 环境下 lucide 系列库**全部无法在 RN 端正常运行**：

| 方案 | 问题 | 现象 |
|------|------|------|
| `lucide-react-taro` v1.5.2 | 内部调用 RN 0.71+ 才有的 `codegenNativeCommands` | RN 端 `TypeError: 0, _reactNative.codegenNativeCommands is not a function` |
| `lucide-react` v0.378.0（fallback） | 依赖 `react-native-svg` 原生模块（未装）+ `import * as` 让 Metro 在 bundling 阶段解析不存在符号 | RN 端 `Property 'Heart' doesn't exist` / `View config getter for component 'path' must be a function` |

**根因**：Taro 3.6.40 模板自带的 React Native 版本是 **0.70**（`Hermes` 引擎 + 旧的 codegen 机制），而 lucide 系列库是为 **Taro 4.x + RN 0.73+** 设计的。

要让 RN 端用真正的 lucide SVG，需装 `react-native-svg` 原生模块并把 Taro 升到 4.x 配套 RN 0.73+——但那是更大的工程，会触发一系列 native 依赖链路问题（`expo-av`、`gesture-handler`、`reanimated` 等同类问题），且会破坏当前 Taro 3.6.40 + RN 0.70 已稳定的 native 工具链。

#### 8.10.2 解决方案：Icon 抽象层 + 平台分层

放弃"全端统一图标库"的目标，改用**调用方无感知的 Icon 抽象层**：

```
调用方统一：<Icon name="search" size={20} color="#0F172A" />
                          │
                          ├── #ifdef H5  → lucide-react 具名 import 渲染 SVG（漂亮）
                          ├── #ifdef RN   → <Text> 渲染 emoji（零原生依赖）
                          └── #ifdef MP   → <Text> 渲染 emoji（同 RN）
```

**`src/components/Icon/index.tsx` 关键设计**：

- **不能用 `import * as Lucide`**：会让 Metro 在 import 阶段就解析整个 lucide 模块，遇到不存在或缺失原生模块的就崩。必须用具名 import（webpack tree-shaking + Metro 分模块解析）。
- **H5 端**：显式列出 40 个常用图标名（`Search` / `Package` / `House` / `User` / `Bell` / `ChevronRight` 等），通过 `H5_ICONS: Record<string, any>` 映射查找。找不到 → fallback 到 emoji。
- **RN/小程序端**：emoji 是原生 Unicode 字符，零依赖直接渲染。`EMOJI_MAP` 覆盖 47 个常用图标。
...
```tsx
// ✅ 正确：统一用 Icon 组件，传 name（kebab-case）
<Icon name="search" size={20} color="#0F172A" />
<Icon name="chevron_right" size={16} color="#94A3B8" />

// ❌ 错误：禁止直接 import lucide
import { Search } from 'lucide-react'  // RN 端会崩
import { Search } from 'lucide-react-taro'  // RN 端 codegenNativeCommands 报错

// ❌ 错误：禁止 className 传色（CSS 变量在 RN/小程序不可靠）
<Icon name="search" className="icon-primary" />
```

#### 8.10.4 已知权衡

- **H5 vs RN 视觉差异**：H5 端用 lucide 线条 SVG（清晰锐利），RN 端用 emoji（彩色但风格不一致）。这是技术妥协，不是设计妥协——emoji 在小尺寸（16-32px）下视觉表现可接受。
- **未来升级路径**：若 Taro 升级到 4.x 配套 RN 0.73+，可直接在 Icon 抽象层把 RN 端从 emoji 切换到 lucide（具名 import 已就位），调用方代码无需改动。
- **新增图标**：如需新增图标，先在 `H5_ICONS`（H5 端）和 `EMOJI_MAP`（RN/小程序端）两边都加条目，保持平台一致性。
...
| v1.4.0 | 2026-09-03 | 批次 5-6 完成 | 主题切换集成（Zustand store + app.tsx 启动初始化）；LucideTaroProvider 全局图标默认色（浅色主题深色线条/深色主题浅色线条） |
...
**目标**：H5 运行时切换深浅色（解决手机深色模式冲突），统一主题管理
...
**修改文件**：
- ✅ `src/app.tsx`（启动时 `initTheme()` + Provider 包裹）
- ✅ `src/pages/my/settings.tsx`（接入 store，移除 DOM 操作）

**关键设计**：
- 主题状态统一在 `useThemeStore`：mode = 'light' | 'dark' | 'system'
- `setTheme()` 自动持久化到 localStorage + 应用到 `<html data-theme>`
- 业务页面用 `useThemeStore((s) => s.mode)` 订阅主题变化
- 解决手机深色模式冲突：保留显式 `[data-theme]` 控制，不被 UA 偏好覆盖

### 批次 6：LucideTaroProvider 全局图标 ✅ 完成

**目标**：统一全局图标默认色（浅色/深色主题下图标线条自动反色）

**修改文件**：
- ✅ `src/app.tsx`（在 `<LucideTaroProvider>` 中传入 `defaultColor`）

**关键设计**：
- 浅色主题：图标默认色 `#0F172A`（深色线条，浅色背景清晰）
- 深色主题：图标默认色 `#F1F5F9`（浅色线条，深色背景清晰）
- 跟随系统：根据 `prefers-color-scheme` 媒体查询动态判断
- 各页面图标可继续用 `color` prop 单独覆盖（自定义语义色）
