...
| 版本 | 日期 | 变更 |
| v1.7.18 | 2026-09-25 | 任务中心与设置页重排：① 新增任务中心页 pages/my/tasks——每日签到 7 格日期条（今天居中、已签实心勾、漏签灰、未来空心）+ 赚分任务列表（API /points/tasks 驱动：daily 任务完成态只看今日流水、次日自动刷新回未完成，once 任务（注册/入驻）看历史永久完成；行动按钮去签到/去纠错/去入驻）；② 我的页积分卡回归原样（撤签到胶囊与连签副文案，一行摆不下显示不全；赚分动作收进任务中心），余额保留真数据；③ 功能区扩两行八格：收藏轴承/关注商家/浏览历史/我的纠错 + 任务中心/我的寻货(占位)/设置/全部功能；④ 设置页重排：音效/震动挪消息组（新消息提醒语义归位），新增权限组与权限管理二级页（相机/相册用途说明+去系统设置，openAppAuthorizeSetting 三端降级兼容），通用组改关于组（启动时自动检查更新简称自动更新、版本更新、ICP 备案行取站点配置点击复制）；⑤ 首页搜索框内扫描/麦克风/相机图标与占位色由硬编码灰改主题色（深色模式原色不可见），快捷三钮渐变底保留品牌色不变（主流做法） |
|------|------|------|
